...
const logger = new Logger(
  `${chalk.gray("[")}${chalk.bold.red("PYPM")}${chalk.gray("]")} `
);
...
const command = yargs()
  ...
  .command(
    ...
    (argv) => {
      ...
      if (fs.existsSync("package.json")) {
        ...
        const version = argv.package.includes("@")
          ? argv.package.split("@")[1]
          : "latest";
        const packageName =
          version === "latest" ? argv.package : argv.package.split("@")[0];
        const installer = spawn(`pip3`, [
          "install",
          "-t",
          "packages",
          "--upgrade",
          argv.package.replace("@", "=="),
        ]);
        ...
        installer.on("exit", (code) => {
          if (code !== null) {
            ...
            packageJson[argv.dev ? "devDependencies" : "dependencies"][
              packageName
            ] = version;
            fs.writeFileSync(
              "package.json",
        ...
        logger.error(
          "Error: You are not in a pypm package! Initialize one with `pypm init`"
        );
      }
    }
  )
  .command(
    ["uninstall <package>", "rm"],
      ...
      if (fs.existsSync("package.json")) {
        ...
        if (argv.dev) {
          if (packageJson.devDependencies[argv.package]) {
            ...
            logger.log(
              "The specified package " + argv.package + " is not installed!"
            );
          }
        } else {
  ...
  .command(
    "config <key> <value>",
    "Modify key-value configuration",
    (yargs) => {
      return yargs
        .positional("key", {
      ...
      if (fs.existsSync("package.json")) {
        ...
        if (argv.script) {
          if (packageJson.scripts[argv.script]) {
            const command =
              packageJson.scripts[argv.script] +
            ...
            logger.line();
            logger.log(
              `> ${packageJson.name}@${packageJson.version} ${argv.script}`
            );
            logger.log(`> ${command}`);
            logger.line();
          ...
          logger.log(
            `${chalk.bold("Package scripts")} inside ${chalk.green(
  ...
  .command(
    "start",
    "Starts your program from the specified main",
    (yargs) => {
      return yargs;
    },
    (argv) => {
  ...
  .command(
    ["vscode", "code"],
    "Sets up proper vscode settings.json for module resolution",
    (yargs) => {
      return yargs;
    },
    (argv) => {
      if (fs.existsSync(".vscode/settings.json")) {
        const settingsJson = JSON.parse(
          fs.readFileSync(".vscode/settings.json").toString("utf-8")
        );

        if (settingsJson["python.analysis.extraPaths"]) {
          if (
            !settingsJson["python.analysis.extraPaths"].includes("./packages")
          )
            settingsJson["python.analysis.extraPaths"].push("./packages");
        } else {
          settingsJson["python.analysis.extraPaths"] = ["./packages"];
        }
        fs.writeFileSync(
